...
PORT = 3602  # Port of the server
save_every = 100 # Save the data every save_every pts
data_name = "Test_Many_Data_Connection.txt"
...
if time_step > 2*max_time_exp: # Time step 2 times bigger than exposition
    ...
    while True:
        ...
        # If a client is connected to the socket, send him the data
        if "client" in locals():
            ...
            # If can't send the data, close the client
            except:
                client.close()
                del client # Remove client from local variables
                print("Disconnected from %s" %(addr[0]))
